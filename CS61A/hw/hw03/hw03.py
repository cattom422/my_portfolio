...
def num_eights(n):
    ...
    if n==8:
        ...
        return num_eights(n%10)+num_eights(n//10)#不能有死循环
...
def interleaved_sum(n, odd_func, even_func):
    ...
    return helper(1) 
#recursion with higher order func
#do not see hint before thinking.           
...
def count_dollars(total):
    ...
    return constrained_count(total, 100)
     """
    #figure out the most common condition,and 
    #compasente based on it.Rather than doing it my way.
    def helper(bill,cur_total):
        if bill>cur_total:
            return helper(next_smaller_dollar(bill),cur_total)
        elif bill==cur_total:
            if bill==1:
                return 1
            else:
                return 1+helper(next_smaller_dollar(bill),cur_total)
        else:
            if bill!=1:
                return helper(next_smaller_dollar(bill),cur_total)+helper(bill,cur_total-bill)
            else:
                return 1
...
def count_dollars_upward(total):
    ...
    def constrained_count(total, smallest_bill):
        ...
        return without_dollar_bill + with_dollar_bill
    return constrained_count(total,5)
#just blocks!
#simple condition:1.use that bill or 2.not.
#why in order?(it's not natural though.I mean upward.)
#the aim is to get easier circumstances!why is that 
